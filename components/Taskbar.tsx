// C:\Users\vppfe\Desktop\Microservice_doc\components\Taskbar.tsx

import React, { useState, useEffect, useRef } from 'react';
import { NavLink } from 'react-router-dom';
import { useAuth } from '../hooks/useAuth'; // Убедись, что useAuth возвращает User с правильным типом
import { UserRole } from '../constants';
import { DashboardIcon, UsersIcon, KeyIcon, LogoutIcon, ScaleIcon, ServerStackIcon } from './icons/IconComponents';
// Возможно, понадобится импорт типа User, если useAuth его не экспортирует явно
// import { User } from '../types'; 

const Taskbar: React.FC = () => {
  const { user, logout } = useAuth(); // Теперь user должен иметь тип User с объектом role
  const [isUserMenuOpen, setIsUserMenuOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);

  const navItems = [
    // Твои элементы меню без изменений
  ...
  // ---> ИСПРАВЛЕНИЕ ЗДЕСЬ <---
  // Используем user?.role?.name для безопасного доступа к имени роли
  // и сравниваем его со значениями UserRole
  const filteredNavItems = navItems.filter(item => 
    user?.role?.name && item.roles.includes(user.role.name as UserRole)
  );
  // ---> КОНЕЦ ИСПРАВЛЕНИЯ <---
  ...
  // ---> НЕБОЛЬШОЕ УЛУЧШЕНИЕ: Добавим проверку на случай, если кнопок нет <---
  const showNav = filteredNavItems.length > 0;
  // ---> КОНЕЦ УЛУЧШЕНИЯ <---

  return (
    <footer className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50">
      {/* Используем rounded-2xl вместо rounded-full для лучшего вида, если кнопок мало */}
      <div className="flex items-center justify-center gap-4 h-16 px-6 bg-black/30 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl">
        
        {/* Показываем навигацию и разделитель, только если есть кнопки */}
        {showNav && (
          <>
        ...
        {/* Меню пользователя (отображается всегда, если user залогинен) */}
        {user && (
           <div className="relative" ref={menuRef}>
            ...
            >
              <img
                ...
                // Используем user.email (он должен быть всегда) для аватара
